...
//GENERATION D'UN JETON
if (isset($_POST['submitToken'])){
  /// Déclaration des données à envoyer au Serveur
  $id = "john";
  $mdp = "password";
  $data = array('id' => $id,'mdp'=>$mdp);
  $data_string = json_encode($data);
  /// Envoi de la requête
    $result = file_get_contents(
  'http://localhost/r401/jeton/serveurJeton.php',
    false,
...
$result = file_get_contents(
    'http://localhost/r401/jeton/serveur.php',
    false,
    stream_context_create(array('http' => array('method' => 'GET', 'header' => "Authorization: Bearer $auth_token"))) // ou DELETE
);
...
<body>
<h1><center>Generation d'un token se passe ici.</center></h1>
<form method="post" action="">
  <input type="hidden" name="id" value="john">
  <input type="hidden" name="mdp" value="password">
  <input type="submit" name = "submitToken" value="Génerer">
</form>
...
<table>
	...
	foreach($r['data'] as $chuck ){
        ...
        '<tr>'.
			...
			'<td><a href="updateChucknFact.php?id='.$chuck['id'].'">Modifier</a></td>'.
